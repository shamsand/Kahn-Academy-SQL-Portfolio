...
select AVG(height_m) AS avg_height, MIN(height_m) AS min_height, MAX(height_m) AS max_height
from marvels;

select AVG(weight_kg) AS avg_weight, MIN(weight_kg) AS min_weight, MAX(weight_kg) AS max_weight
from marvels;

select *
from marvels
group by hometown
having height_m >= avg(height_m);

select name, hometown, popularity
from marvels
where gender = "Male" AND alignment = "Good"
order by popularity desc;

select name,
case
    when intelligence < 3 then "Dumb"
    when intelligence > 6 then "Smart"
    else "Average"
end as intelligence2
from marvels;
